...
void entree_sortie_lire_jeu_fichier(const char* filename,jeu_siam* jeu)
{
    ...
    jeu_siam nouveau_jeu;
    jeu_initialiser(&nouveau_jeu);
    int  *nouveau_joueur=&nouveau_jeu.joueur;
    ...
    if(jeu_etre_integre(&nouveau_jeu)==0);
    else printf("Jeu invalide lecture fichier %s\n",filename);
}
